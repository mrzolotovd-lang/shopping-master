...
class NLPPatterns:
    ...
    PURCHASE_PATTERNS = [
        ...
        r"купил\s+(?P<item>[а-яА-ЯёЁ]+)\s+(?P<amount>\d+(?:\.\d+)?)\s*(?P<unit>л|кг|г|мл|шт|упак|упаковка)?",
        # "купил 5 йогуртов" / "купил 10 яиц"
        r"купил\s+(?P<amount>\d+(?:\.\d+)?)\s+(?P<item>[а-яА-ЯёЁ]+)",
        # "купил молоко" (без количества)
        r"купил\s+(?P<item>[а-яА-ЯёЁ]+)",
        # "приобрёл молоко 2 литра"
        r"приобрёл\s+(?P<item>[а-яА-ЯёЁ]+)\s+(?P<amount>\d+(?:\.\d+)?)\s*(?P<unit>л|кг|г|мл|шт|упак|упаковка)?",
        # "добавь молоко"
        r"добавь\s+(?P<item>[а-яА-ЯёЁ]+)\s+(?P<amount>\d+(?:\.\d+)?)\s*(?P<unit>л|кг|г|мл|шт|упак|упаковка)?",
        # "взял молоко" (colloquial)
        r"взял\s+(?P<item>[а-яА-ЯёЁ]+)\s+(?P<amount>\d+(?:\.\d+)?)\s*(?P<unit>л|кг|г|мл|шт|упак|упаковка)?",
    ]

    UPDATE_PATTERNS = [
        # "молока осталось 2 литра" / "молока осталось 2л"
        r"(?P<item>[а-яА-ЯёЁ]+)\s+осталось\s+(?P<amount>\d+(?:\.\d+)?)\s*(?P<unit>л|кг|г|мл|шт)?",
        # "молока осталось половина"
        r"(?P<item>[а-яА-ЯёЁ]+)\s+осталось\s+(?P<level>половина|пол-упаковки|пол упаковки)",
        # "молоко закончилось" / "молоко нет" / "кефир кончился"
        r"(?P<item>[а-яА-ЯёЁ]+)\s+(?P<level>почти\s+закончилось|закончилось|кончилось|нет)",
        # "сахара совсем мало" / "сахара мало" / "сахара немного"
        r"(?P<item>[а-яА-ЯёЁ]+)\s+(?P<level>совсем\s+мало|почти\s+нет|мало|немного|много|почти\s+полная|чуть[\s-]?чуть)",
        # "молока на донышке" (idiomatic)
        r"(?P<item>[а-яА-ЯёЁ]+)\s+на\s+донышке",
    ]
    ...
    STATUS_PATTERNS = [
        ...
        r"сколько\s+осталось\s+(?P<item>[а-яА-ЯёЁ]+)",
        # "статус молоко"
        r"статус\s+(?P<item>[а-яА-ЯёЁ]+)",
        # "покажи молоко"
        r"покажи\s+(?P<item>[а-яА-ЯёЁ]+)",
        # "покажи все товары"
        r"покажи\s+все\s+товары",
        # "что у нас есть"
        r"что\s+у\s+нас\s+есть",
        # "есть ли молоко"
        r"есть\s+ли\s+(?P<item>[а-яА-ЯёЁ]+)",
    ]
